...
router.post('/', auth, adminOnly, async (req, res) => {
  try {
    ...
    await run(
      ...
      [id, name, email, hashed, role, branch_id || null],
    );
    res.status(201).json({ id, name, email, role, branch_id });
  } catch (err) {
...
router.put('/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    if (password) {
      const hashed = await bcrypt.hash(password, 10);
      await run(
        'UPDATE users SET name=?, email=?, role=?, branch_id=?, is_active=?, password=? WHERE id=?',
        [name, email, role, branch_id || null, is_active !== undefined ? is_active : 1, hashed, req.params.id],
      );
    } else {
      await run(
        'UPDATE users SET name=?, email=?, role=?, branch_id=?, is_active=? WHERE id=?',
        [name, email, role, branch_id || null, is_active !== undefined ? is_active : 1, req.params.id],
      );
    }
    res.json(await one(
      `SELECT u.id, u.name, u.email, u.role, u.is_active, b.name AS branch_name
